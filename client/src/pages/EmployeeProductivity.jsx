...
import {
  UserGroupIcon, CalendarDaysIcon, ChevronDownIcon,
} from '@heroicons/react/24/outline';
import EmployeeProductivityReport from '../components/EmployeeProductivityReport';

// ============================================================================
// PMS → Employee Productivity (Service Engineer Productivity)
// Labour-file SR NUMBERs matched against the 'Response Time & MaxTTR Details'
// import; each matched SR counts once for its SE NAME on its SR CLOSE DATE.
// This page owns the reporting period; the table itself (view / columns /
// cells / sort / search / export) lives in components/EmployeeProductivityReport.
// Backend: GET /pms/report/employee-productivity (master admin only)
// ============================================================================
...
const EmployeeProductivity = () => {
  ...
  // Reporting period — applied range (ISO) + the range-picker popover, same
  // pattern as the Sales & Labour report's period picker.
  ...
  // `preloaded`, so the matched date range is known here (it bounds the
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      setData(d);
      if (!quiet) toast.success('Employee Productivity loaded');
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
        ...
        <div className="rounded-2xl px-3 sm:px-5 py-3 mb-3 text-white relative"
          ...
          <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="text-lg sm:text-xl font-bold leading-tight">Employee Productivity Report</h1>
                <p className="text-[11px] text-white/70 leading-tight">
                  {dataRange.max
                    ? <>Service Engineer productivity · data {fmtDayYr(dataRange.min)} → {fmtDayYr(dataRange.max)}</>
                    : (loading ? 'Loading…'
                      : 'No matched data yet — upload the Labour Revenue (PMS) and Response Time & MaxTTR files')}
                </p>
              </div>
            </div>

            {/* Period picker — opens on hover, click still toggles (same as the
              Sales & Labour report's picker). */}
