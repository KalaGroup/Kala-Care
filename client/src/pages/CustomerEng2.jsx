...
const CustomerEng2 = () => {
  ...
  const filteredCustomers = useMemo(() => {
    ...
    // Flag filter (also handles status filters: WIP, F, R = Rejected, NC = Not connected)
    if (selectedFlag !== "all") {
      if (selectedFlag === 'WIP') {
        // Work in Progress — latest status is "wip"
        filtered = filtered.filter(c => c.latest_status === 'wip');
      } else if (selectedFlag === 'F') {
        // Followup — latest status is "rescheduled"
        ...
        // All statuses — WIP + F + NC + R combined
        filtered = filtered.filter(c =>
  ...
  // F (Followup) count — loaded customers whose latest status is "rescheduled"
  ...
  const handleSubmitFollowup = async (e) => {
    ...
    for (const campaignId of selectedCampaignsForFollowup) {
      ...
      if (campaignData.status === 'rescheduled' || campaignData.status === 'not_connected' || campaignData.status === 'wip') {
        ...
        if (!nextDate) {
          if (campaignId === 'other') {
            toast.error(`Please select Next Follow-up Date for "${campaignData.status === 'wip' ? 'WIP' : 'Followup'}" status in Post Warranty follow-up`);
          } else {
            const campaign = customerCampaigns.find(c => c.id === parseInt(campaignId));
            toast.error(`Please select Next Follow-up Date for "${campaignData.status === 'wip' ? 'WIP' : 'Followup'}" status in drive: ${campaign?.name}`);
          }
          return;
        }
        ...
        if (!flag && campaignData.status !== 'not_connected') {
          if (campaignId === 'other') {
            toast.error(`Please select Follow-up Flag for "${campaignData.status === 'wip' ? 'WIP' : 'Followup'}" status in Post Warranty follow-up`);
          } else {
            const campaign = customerCampaigns.find(c => c.id === parseInt(campaignId));
            toast.error(`Please select Follow-up Flag for "${campaignData.status === 'wip' ? 'WIP' : 'Followup'}" status in drive: ${campaign?.name}`);
          }
          return;
        }
      }
      ...
      if (campaignId !== 'other' && (campaignData.status === 'completed' || campaignData.status === 'wip')) {
        ...
        if (campaignData.status === 'rescheduled') {
          if (campaignData.quotation_sent) {
            const campaign = customerCampaigns.find(c => c.id === parseInt(campaignId));
            toast.error(`Cannot save: Status is "Followup" but quotation is sent for drive "${campaign?.name}". Please uncheck "Quote Sent" or change status.`);
            return;
          }
...
Dear Sir/Madam,<br><br>Please find attached the letter${f.subject ? ` regarding <strong>${escapeLetterHtml(f.subject)}</strong>` : ''}${f.ref_no ? ` (Ref No: ${escapeLetterHtml(f.ref_no)})` : ''}.<br><br>Best regards,<br>${escapeLetterHtml(COMPANY_FULL)}</div>`;
  ...
  /* Did the Letter Master lock this box? An unset flag means editable, so a
     format saved before the locks existed behaves exactly as before. */
...
const selectLetterFormat = async (fmt) => {
  ...
  // "Label: value" is short enough to share one line three-across inside the letter's
  // text column (PAIR_FIT_LIMIT is derived from it); otherwise 2 per row.
  const buildReferencesHtml = () => {
    ...
    const cell = (p) => p
      ? `<td style="padding:2px 10px 2px 0;white-space:nowrap;vertical-align:top;"><strong>${escapeLetterHtml(p.label)}:</strong></td>
         <td style="padding:2px 16px 2px 0;vertical-align:top;overflow-wrap:anywhere;">${escapeLetterHtml(p.value)}</td>`
      : `<td></td><td></td>`;
    let trs = '';
    ...
    return `<div style="margin-top:6px;color:#333;font-size:12px;">
      <table style="width:100%;border-collapse:collapse;font-size:12px;line-height:1.6;">${trs}</table>
    </div>`;
  };
...
${renderLetterParaText(f.start_para)}`;
  ...
  const handlePrintLetter = async () => {
    ...
    try {
      ...
      // The attachments ride along in the SAME print job — each image centred
      // on its own A4 sheet (shrunk to fit), each PDF page on its own sheet.
      // Rendered pages are memoised, so re-printing is instant.
      let attHtml = '';
  ...
  const handleSendLetter = async () => {
    ...
    try {
      // The letter PDF carries every attachment merged in as pages, so the
      // customer receives ONE file (see generateLetterPdfBase64).
      ...
      // Original files ride along flagged skip_email — stored on the record for
      // the history view/print, but not emailed separately (the letter PDF
      // already carries them). If the PDF failed, they go out separately as before.
  ...
  const printViewedLetter = async () => {
    ...
    const body = viewLetterBareHtml.replace(/<img\b[^>]*?(?:max-width\s*:\s*780px|width\s*:\s*100%)[^>]*?>/gi, '');
    // viewLetterAttachments is already cleaned in viewLetterRecord; the letter's
    // own attachments print in the same job — one A4 page per image / PDF page.
    ...
    const boxesContent = {
      branch: (
        <div className="p-3 sm:p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-40">
      ...
      asset: (
        <div className="p-3 sm:p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 gap-y-3">
            <div className="space-y-0.5">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-3">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3 mb-1.5">
            ...
            <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-end gap-2">
              ...
              <div
                className="flex flex-wrap items-center gap-1 max-w-full whitespace-nowrap"
          ...
          <div className="bg-white rounded-xl shadow-sm px-3 py-1 mb-1.5">
            <div className="flex items-center justify-between gap-3 max-md:flex-wrap">
              ...
              >
                ...
                {/* ALL statuses — WIP + F + NC + R combined (styled like the flag-count All button) */}
                <button
                  onClick={() => handleFlagSelect("ALL_STATUS")}
                  title="All statuses — WIP + F + NC + R combined"
                ...
                {/* F (Followup) — latest status is "rescheduled" */}
                <button
                  onClick={() => handleFlagSelect("F")}
                  title="Followup — latest status is Followup"
                  className={`px-2 py-0.5 text-[13px] whitespace-nowrap font-semibold rounded-md transition-colors ${selectedFlag === "F"
                    ? "bg-[#2f3192] text-white shadow-sm"
                    : "text-black hover:bg-gray-50"
                    }`}
                >
                            ...
                            <tr
                              ...
                              {columnOrder.map(colId => {
                                ...
                                if (colId === 'campaign_status') {
                                  ...
                                  else if (customer.latest_status === 'rescheduled') statusTitle = 'Followup';
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              if (campaignData.quotation_sent && (newStatus === 'rescheduled' || newStatus === 'not_connected')) {
                                const campaign = customerCampaigns.find(c => c.id === parseInt(campaignId));
                                const lbl = newStatus === 'not_connected' ? 'Not Connected' : 'Followup';
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (campaignData.status === 'rescheduled' || campaignData.status === 'not_connected') {
                                  toast.error('Cannot send quotation when status is "Followup" or "Not Connected"');
                                  return;
                                }
