...
const KnowledgeBook = () => {
    ...
    const handleUpload = async (e) => {
        ...
        // Block files over the size limit before uploading; upload the rest.
    ...
    const removeFile = async (file) => {
        const res0 = await Swal.fire({
            ...
            icon: 'warning', showCancelButton: true, confirmButtonColor: 'rgb(2, 18, 39)', cancelButtonColor: '#3085d6',
            confirmButtonText: 'Yes, delete',
        });
        if (!res0.isConfirmed) return;
        try {
    ...
    // Download a whole folder (and everything inside it) as one .zip.
    // Uses fetch so the auth headers go along; available to every logged-in user.
        ...
        <div className="min-h-screen font-sans">
            <div className="max-w-7xl mx-auto px-3 sm:px-5 pb-10">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-9 gap-0 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
                        {/* ===== Folders: classic yellow icons ===== */}
                        {shownFolders.map((f) => (
                            <div key={f.id}
                                className={`group flex flex-col h-full rounded-xl p-2.5 transition hover:bg-indigo-50/60 ${isAdmin && f.is_hidden ? 'opacity-60' : ''}`}>
                                ...
                                {isAdmin && (
                                    <div className="mt-auto pt-1.5 flex items-center justify-center gap-1">
                                        <button onClick={() => downloadFolder(f)} title="Download as ZIP"
                        ...
                        {shownFiles.map((file) => {
                            ...
                            return (
                                <div key={file.id} className="group flex flex-col h-full rounded-xl p-2.5 transition hover:bg-gray-50">
                                    <button onClick={() => setPreview(file)} className="w-full flex flex-col items-center" title={file.name}>
                                        {file.kind === 'image' ? (
                                            ...
                                            <span className="w-16 h-16 rounded-lg flex items-center justify-center transition-transform group-hover:scale-105"
                                                style={{ backgroundColor: 'rgb(165, 226, 250)' }}>
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm divide-y divide-gray-200">
                        ...
                        <div className="grid grid-cols-12 divide-x divide-gray-200 bg-gray-50 text-[10px] sm:text-[11px] font-semibold text-black uppercase tracking-wider">
                            <div className="col-span-4 px-3 py-1.5">Name</div>
                            <div className="col-span-2 px-3 py-1.5">Type</div>
                            <div className="col-span-2 px-3 py-1.5">Size</div>
                            <div className="col-span-2 px-3 py-1.5">Modified</div>
                        ...
                        {shownFolders.map((f) => (
                            <div key={f.id}
                                ...
                                <button onClick={() => openFolder(f)} className="col-span-4 flex items-center gap-2 min-w-0 text-left px-3 py-1.5">
                                ...
                                <div className="col-span-2 px-3 py-1.5 flex items-center text-gray-400 truncate">{prettyDate(f.modified_at)}</div>
                                <div className="col-span-2 px-3 py-1.5 flex items-center justify-end gap-1">
                                    <button onClick={() => downloadFolder(f)} className="rounded-lg p-1 text-gray-500 hover:bg-gray-100 transition" title="Download as ZIP">
                                        <ArrowDownTrayIcon className="h-3.5 w-3.5" />
                                    </button>
                                    {isAdmin && (
                                        <>
                        ...
                        {shownFiles.map((file) => {
                            ...
                            return (
                                <div key={file.id} className="grid grid-cols-12 divide-x divide-gray-200 text-[11px] sm:text-xs items-stretch hover:bg-indigo-50/40 transition">
                                    <button onClick={() => setPreview(file)} className="col-span-4 flex items-center gap-2 min-w-0 text-left px-3 py-1.5">
                                    ...
                                    <div className="col-span-2 px-3 py-1.5 flex items-center text-gray-400 truncate">{prettyDate(file.modified_at)}</div>
                                    <div className="col-span-2 px-3 py-1.5 flex items-center justify-end gap-1">
