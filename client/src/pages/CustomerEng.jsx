...
To ensure uninterrupted service and optimal performance of your equipment, we request you to renew your maintenance contract before the due date.`;
    ...
    const buildServiceCycleHtml = () => {
        ...
        const cellStyle = (key, last) => {
            const w = LETTER_COL_WEIGHTS[key] || 1;
            // No top padding; keep bottom padding (9px) so the last line never touches the
            // row border in html2canvas (PDF / print), which ignores vertical centering.
            return `flex:${w} 1 0;min-width:0;display:flex;align-items:flex-start;justify-content:flex-start;` +
                `text-align:left;padding:0 8px 9px;box-sizing:border-box;line-height:1.5;white-space:pre-wrap;` +
                `word-break:break-word;overflow-wrap:anywhere;${last ? '' : `border-right:1px solid ${B};`}`;
        };
