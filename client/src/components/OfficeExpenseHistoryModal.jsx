...
const OfficeExpenseHistoryModal = ({
  ...
  const handlePaidChange = (id, value) => {
    ...
    const t = setTimeout(async () => {
      try {
        await axios.put(`${API_BASE_URL}/office-expenses/history/${id}/paid-date`, { paid_date: value || null });
  ...
  const applyBulkPaid = async () => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/office-expenses/history/bulk-paid-date`, { record_ids: ids, paid_date: bulkPaid });
      ...
      toast.success(`Applied to ${ids.length} record(s)`);
    } catch {
  ...
  // ── TALLY IMPORT ──
  // One Journal Voucher per OE voucher, mirroring the physical Expenses Voucher:
  // Dr each Expense Head (GL Code) ledger with its summed amount,
  ...
  const runTallyImport = async (targetGroups) => {
    ...
    try {
      const results = await Promise.all(targetGroups.map(g =>
        axios.get(`${API_BASE_URL}/office-expenses/history/voucher-records`, {
          params: { submit_voucher_no: g.submit_voucher_no, branch_code: g.branch_code || undefined },
      ...
      results.forEach(({ g, recs }) => {
        ...
        const map = {};
        recs.forEach(r => {
          const head = String(r.expenses_head || '').trim() || 'Office Expenses';
          map[head] = (map[head] || 0) + (parseFloat(r.amount) || 0);
        });
        ...
        const narration = from && to
          ? `Office expense from ${from} to ${to}`
          : `Office expense — Voucher ${g.submit_voucher_no}`;
        vouchers.push({ voucherNo: g.submit_voucher_no, narration, debits, creditLedger });
      });
      if (!vouchers.length) return;
      const label = vouchers.length === 1 ? vouchers[0].voucherNo : `OE_${vouchers.length}_vouchers`;
  ...
  const recTotal = useMemo(
    () => records.reduce((s, r) => s + parseFloat(r.amount || 0), 0), [records]);
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-[98vw] max-h-[92vh] overflow-hidden flex flex-col">
        ...
        <div className="px-4 py-3 flex justify-between items-center shrink-0 max-md:px-2 max-md:flex-wrap max-md:gap-2" style={{ background: themeColor }}>
          <div className="flex items-center gap-2 max-md:flex-wrap">
            ...
            <h2 className="text-sm font-semibold text-white">Office Expense History — Voucher Wise</h2>
            {!loading && (
          ...
          <div className="shrink-0 px-4 py-2 border-b bg-gray-100 flex items-center gap-2 flex-wrap max-md:px-2">
            {!selected ? (
              ...
              <>
                ...
                {canExport && (
                  <button
                    onClick={() => exportToExcel(
                      records,
                      `oe_history_voucher_${selected.submit_voucher_no}_${selected.branch_code}.xlsx`,
                      [
                ...
                {/* Bulk paid-date for whole voucher */}
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-emerald-50 border border-emerald-200 max-md:flex-wrap">
        ...
        <div className="flex-1 overflow-auto" style={{ scrollbarWidth: 'thin' }}>
          {loading ? (
            ...
            /* ── VOUCHER RECORDS (verified-by + editable HO paid date) ── */
            <table className="border-collapse w-full" style={{ minWidth: '1700px' }}>
              <thead className="sticky top-0 z-10"><tr style={{ backgroundColor: '#f0f1ff' }}>
                {['Sr.', 'Branch', 'Internal Branch', 'Date', 'Exps. Sub Head', 'Expense Head (GL Code)', 'Description', 'Paid To', 'Invoice No.', 'Amount (₹)', 'Voucher No.', 'Paid By', 'Verified By', 'Submitted By', 'HO Paid Date'].map((c, i) => (
              ...
              <tfoot className="sticky bottom-0"><tr style={{ backgroundColor: '#f0f1ff' }}>
                <td colSpan={9} className="px-3 py-1.5 text-[11px] font-bold text-gray-600 text-right border-t-2 border-gray-200">Grand Total</td>
...
export default OfficeExpenseHistoryModal;