...
const EmployeeActivityModal = ({
    ...
    activityName,        // the activity that was clicked
    apiBaseUrl,
    timePeriod,
    customStartDate,
    customEndDate,
    selectedCampaigns = [],   // campaign IDs selected in Dashboard
    allCampaigns = []         // full list to resolve id -> name
}) => {
    // Build a Set of selected campaign NAMES (followups carry campaign_name, not id)
    // If nothing is selected in Dashboard, default to ALL campaigns (same as Activity tab behavior)
    const selectedCampaignNames = useMemo(() => {
        const set = new Set();
        if (selectedCampaigns && selectedCampaigns.length > 0) {
            // User picked specific campaigns
            ...
            // No selection → include every campaign by default
            allCampaigns.forEach(c => {
                set.add((c.name || '').toLowerCase());
            });
        }
    ...
    // SAME endpoint as MyPerformance — but with the clicked employee's identity
    ...
    // Filter rows by the clicked activity + selected campaigns + search + date range
    const filteredFollowups = useMemo(() => {
        if (!allFollowupsData) return [];

        return allFollowupsData.filter(fu => {
            // ✅ Filter by clicked activity
            if (activityName) {
                const act = (fu.activity_content || '').toLowerCase();
                if (!act.includes(activityName.toLowerCase())) return false;
            }

            // ✅ Filter by campaigns (selected in Dashboard, or all by default)
            // Only apply if we actually have a campaign list loaded
    ...
    }, [allFollowupsData, activityName, selectedCampaignNames, debouncedSearch, createdFromDate, createdToDate]);
                ...
                >
                    <div>
                        <h3 className="text-base font-semibold text-white">
                            {activityName || 'Activity'} — {employee?.user_name || 'Employee'}
                        </h3>
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    if (createdToDate && v && new Date(createdToDate) < new Date(v)) {
                                        setCreatedToDate('');
                                    }
                                }}
