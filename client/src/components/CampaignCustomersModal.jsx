...
const CampaignCustomersModal = ({ isOpen, onClose, campaign, apiBaseUrl, userData }) => {
    ...
    const fetchCampaignCustomers = async () => {
        try {
            ...
            const response = await fetch(`${apiBaseUrl}/performance/campaign-customers/${campaign.campaign_id}`, {
                method: 'POST',
                headers: {
            ...
            if (data.customers && Array.isArray(data.customers)) {
                // Sort customers by Last Follow-up Date (most recent first)
                const sortedCustomers = [...data.customers].sort((a, b) => {
                    ...
                    // Put customers with no follow-up date at the bottom
    ...
    const exportToExcel = () => {
        ...
        wsData.push(['Campaign Name:', campaignInfo?.campaign_name || 'N/A']);
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Campaign Customers');
        ...
        const filename = `campaign_${campaignInfo?.campaign_name?.replace(/\s/g, '_')}_customers_${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.xlsx`;
    ...
    const getStatusBadge = (status) => {
        const styles = {
            completed: { text: '#000000', icon: '✓' },
            wip: { text: '#000000', icon: '⟳' },
            rejected: { text: '#000000', icon: '✗' },
            rescheduled: { text: '#000000', icon: '⟳' },
            pending: { text: '#000000', icon: '○' }
        };
        const style = styles[status?.toLowerCase()] || styles.pending;
        const formattedStatus = status ? status.charAt(0).toUpperCase() + status.slice(1).toLowerCase() : status;

        return (
            <span className="inline-flex items-center justify-center gap-1 px-1.5 py-0.5 rounded-full text-[11px] font-medium" style={{ backgroundColor: style.bgLight, color: style.text }}>
                {formattedStatus}
            </span>
        );
    };
                ...
                <div className="relative w-screen h-screen max-w-none bg-white shadow-2xl overflow-hidden rounded-none">
                        ...
                        <div className="relative flex items-center justify-between gap-2">
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5">
                                    ...
                                    <h2 className="text-base sm:text-lg font-bold text-white tracking-tight truncate">{campaignInfo?.campaign_name}</h2>
                                </div>
                                <div className="flex flex-wrap gap-1.5 sm:gap-2 text-white/80 text-[10px] sm:text-xs">
                                    ...
                                    <span className="flex items-center gap-0.5">
                                        ...
                                        {campaignInfo?.total_customers} Customers Remaining
                                    </span>
                                </div>
                            </div>
                            <button
                    ...
                    <div className="relative bg-gray-50">
                        ...
                        <div
                            ...
                            style={{ maxHeight: 'calc(100vh - 250px)', minHeight: '450px', overflowX: 'auto' }}
                        >
                            {loading ? (
                                    ...
                                    <tbody className="bg-white">
                                        {filteredCustomers.map((customer, idx) => (
                                            <tr key={customer.instance_id || idx} className="hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td className="px-2 py-1 text-center border border-gray-200">
                                                    <div className="text-[11px] font-medium text-black">{highlightText(customer.last_followup_user_name, searchTerm)}</div>
                                                    <div className="text-[10px] text-black">{highlightText(customer.last_followup_user_id, searchTerm)}</div>
                                                </td>
                                                ...
                                                <td className="px-2 py-1 text-center border border-gray-200">
                                                    <div className="flex justify-center">
                                                        <span className="inline-flex items-center justify-center px-1.5 py-0.5 rounded-md text-[10px] font-bold" style={{ color: "black" }}>
...
export default CampaignCustomersModal;