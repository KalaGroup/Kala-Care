...
const SOURCE_META = {
  service_engineer: {
    short: 'SE', label: 'Service Engineer', accent: '#1e40af', bg: '#dbeafe',
    amountKey: 'total_amount',
    groupKey: (r) => (r.service_engineer_name || r.engineer_name || 'Unknown'),
  },
  sales: {
    short: 'S&BM', label: 'Sales & BM', accent: '#7c3aed', bg: '#f3e8ff',
    amountKey: 'total_amount',
...
/**
 * View-only branch history modal (voucher → engineer → records drill-down).
 * No paid-date editing — paid date is shown read-only.
 *
 * Props:
 *  - branchCode  (string)  e.g. "420435_1"
 *  - branchName  (string)  e.g. "Ch.Sambhaji Nagar"
 *  - themeColor  (string)
 *  - canExport   (bool)    show/hide Export buttons
 *  - onClose     (fn)
 */
const BranchTADAHistory = ({ branchCode, branchName, themeColor, onClose, canExport = false }) => {
    ...
    (async () => {
      ...
      try {
        const [seR, seG, sbR, sbG, bwR, bwG] = await Promise.all([
          axios.get(`${API_BASE_URL}/tada-ho/branch-history`, { params: { branch_code: branchCode } }),
          axios.get(`${API_BASE_URL}/tada-ho/branch-history-vouchers`, { params: { branch_code: branchCode } }),
          axios.get(`${API_BASE_URL}/tada-salesbm/history`, { params: { branch_code: branchCode } }).catch(() => ({ data: [] })),
          axios.get(`${API_BASE_URL}/tada-salesbm/history/grouped`, { params: { branch_code: branchCode } }).catch(() => ({ data: { groups: [] } })),
          axios.get(`${API_BASE_URL}/tada-bill-wise/history`, { params: { branch_code: branchCode } }).catch(() => ({ data: [] })),
          axios.get(`${API_BASE_URL}/tada-bill-wise/history/vouchers`, { params: { branch_code: branchCode } }).catch(() => ({ data: { groups: [] } })),
        ]);
        if (!alive) return;
        setData({
  ...
  const exportVoucherList = () => {
    ...
    XLSX.writeFile(wb, `History_${isAll ? 'All' : SOURCE_META[activeTab].short}_${branchName || ''}.xlsx`);
  };
  ...
  // Read-only paid-date status badge (voucher level)
  const paidStatus = (g) => {
    if (g.paid_date) {
      return (
    ...
    return <span className="text-[10px] text-gray-400 italic">Not paid yet</span>;
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-[98vw] max-h-[92vh] overflow-hidden flex flex-col">
        ...
        <div className="px-4 py-2 flex justify-between items-center shrink-0 gap-3 flex-wrap max-md:px-2 max-md:gap-2" style={{ background: themeColor }}>
          <h2 className="text-sm font-semibold text-white whitespace-nowrap">History — {branchName}</h2>
          {!loading && (
        ...
        <div className="flex-1 overflow-auto" style={{ scrollbarWidth: 'thin' }}>
          {loading ? (
            ...
            filteredGroups.length === 0 ? (
              ...
              <table className="border-collapse w-full max-md:min-w-[1000px]">
                <thead className="sticky top-0 z-10">
                  <tr style={{ backgroundColor: '#f0f1ff' }}>
                    {['Sr. No.', ...(isAll ? ['Source'] : []), 'Voucher No.', 'Submitted By', 'Verified By', 'Period', 'Engineers', 'Records', 'Total Amount', 'Paid Date'].map((h, i) => (
                  ...
                  {filteredGroups.map((g, idx) => {
                    ...
                    return (
                      <tr key={key} className="hover:bg-blue-50 transition-colors" style={{ height: '40px' }}>
                        ...
                        <td className="px-3 py-1 border border-gray-200 text-[12px] text-center">{paidStatus(g)}</td>
                      </tr>
                    );
                  })}
                </tbody>
                  ...
                  <tbody>
                    {selectedEngineer.records.map((rec, idx) => (
                      <tr key={rec.id} className="hover:bg-blue-50/30" style={{ height: '34px' }}>
                        ...
                        <td className="px-2 py-0.5 border border-gray-300 text-[11px] text-center font-semibold text-green-700">
                          {rec.paid_date
                            ? new Date(rec.paid_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })
                            : <span className="text-gray-400 italic font-normal">Not paid</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
export default BranchTADAHistory;